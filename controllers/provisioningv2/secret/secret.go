...
// TODO: fix this redefinition from github.com/rancher/rancher/pkg/controllers/management/rbac/rbac.go
...
func (h *handler) OnSecret(key string, secret *v1.Secret) (*v1.Secret, error) {
	...
	creatorID, ok := secret.Annotations[CreatorIDAnn]
	if !ok || creatorID == "" {
		return secret, nil
	}
	...
	s := secret.DeepCopy()
	delete(s.Annotations, CreatorIDAnn)
